...
def run_episode(env, agent, rendering=True, max_timesteps=1000):
    ...
    while True:
        ...
        #print('shaep of state',state.shape)
        state_history[0,:,:,0:history_length-1] = state_history[0,:,:,1:]
        state_history[0,:,:,-1] = state
        #state_history = np.expand_dims(state, axis = 3)
        #print('shape of history state',state_history.shape)
        ...
        a = agent.sess.run(agent.logits, feed_dict={agent.x_input:state_history})[0]        
       
        next_state, r, done, info = env.step(a)#change from b to a   
        episode_reward += r       
        state = next_state
        step += 1
...
if __name__ == "__main__":
    ...
    # TODO: load agent
    agent = Model(learning_rate=0.0001339, history_length=1)
